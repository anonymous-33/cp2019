...
#    Copyright 2019
#
#    This version of the software is distributed with this License only for the blind review
#    of the CP-2019 conference (http://cp2019.a4cp.org/). After the review the software will be distributed
#    with the Apache License extended for the 2019 with its real authors.
...
class SmtLibParser(object):
    ...
    def __init__(self, environment=None, interactive=False):
        ...
        self.commands = {smtcmd.ASSERT : self._cmd_assert,
                        smtcmd.ASSERT_SOFT: self._cmd_assert_soft, #--
                         ...
                         smtcmd.MAXMIN: self._cmd_maxmin, #--
                         smtcmd.MINMAX: self._cmd_minmax, #--
                         smtcmd.MAXIMIZE : self._cmd_maximize, #---
                         smtcmd.MINIMIZE : self._cmd_minimize, #---
    ...
    def parse_all_params(self,tokens,command): #
        """Parses a list of types from the tokens"""
        self.consume_opening(tokens, command)
        current = tokens.consume()
        res = []
    ...
    def _cmd_set_option(self, current, tokens):
        """(set-option <option>)"""
        elements = self.parse_atoms(tokens, current, 3) #
        return SmtLibCommand(current, elements)
    ...
    def _cmd_maxmin(self,current,tokens): #---
    ...
    def _cmd_minmax(self,current,tokens): #---
