...
BASE_DIR_ML="${BASE_DIR}/bench/machine_learning/"
...
function apply_file () {
    ...
    do
       ...
       eval "{ /usr/bin/time -v timeout --signal=SIGKILL 10m ourtool162 $smt2_file >> $DST_DIR/ourtool_smt2.txt; } 2>> $DST_DIR/ourtool_smt2.txt";
       # g12-mip
       ...
       eval "{ /usr/bin/time -v timeout --signal=SIGKILL 10m emzn2fzn.py -O- --fzn ourtool-$file_fzn.fzn $mzn_file >> $DST_DIR/ourtool_flat.txt; } 2>> $DST_DIR/ourtool_flat.txt";
       echo "### ---- ourtool-fzn-opz3 ---- ###" >> "$DST_DIR"/ourtool_flat.txt;
       eval "{ /usr/bin/time -v timeout --signal=SIGKILL 10m ourtool162 -opt.fzn.bv_integers=False -opt.fzn.ite_encoding=True -input=fzn < ourtool-$file_fzn.fzn >> $DST_DIR/ourtool_flat.txt; } 2>> $DST_DIR/ourtool_flat.txt";
       # ourtool-fzn-def
       echo "## ---- $path_file_mzn ---- ##" >> "$DST_DIR"/ourtool_flat_def.txt;
       echo "### ---- mzn2fzn ---- ###" >> "$DST_DIR"/ourtool_flat_def.txt;
       eval "{ /usr/bin/time -v timeout --signal=SIGKILL 10m mzn2fzn -O- --fzn ourtool-def-$file_fzn.fzn $mzn_file >> $DST_DIR/ourtool_flat_def.txt; } 2>> $DST_DIR/ourtool_flat_def.txt";
       echo "### ---- ourtool-fzn-opz3 ---- ###" >> "$DST_DIR"/ourtool_flat_def.txt;
       eval "{ /usr/bin/time -v timeout --signal=SIGKILL 10m ourtool162 -opt.fzn.bv_integers=False -opt.fzn.ite_encoding=True -input=fzn < ourtool-def-$file_fzn.fzn >> $DST_DIR/ourtool_flat_def.txt; } 2>> $DST_DIR/ourtool_flat_def.txt";
